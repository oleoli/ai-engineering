...
async def search(request: Request, payload: RetrievalRequest) -> RetrievalResult:
    """Return chunks for the embedded query under the resolved retrieval config.

    The two S10 switches are resolved by precedence: request param → runtime
    override → .env default. ``search_mode``/``rerank`` left null on the request
    fall through to the hot runtime config and finally the settings defaults."""
